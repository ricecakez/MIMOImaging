...
for nr = 1:Nr
    for k = 1:K
        ...
        t = t(:);
        x1 = exp(-1i*2*pi*Fc*t).*RxSig(((L_c+1):L_T),nr,k);%RxSig((L_c+1):L_T,nr);   %去CP
        tmp(:,k) = fft(x1)/N;  %解调，解码
    end
    for n = 1:N
        tmpI(n,:) = xcorr(tmp(n,:),a);
    end
    for nt = 1:Nt
        nv = (nr-1)*Nt+nt;
        tau0(nv) = 2*Rv0(nv)/c;
        tmp0 = diag(exp(1i*2*pi*((nt:Nt:N)-1)*df*(tau0(nv)-T_min)))*tmpI(nt:Nt:end,:)...
