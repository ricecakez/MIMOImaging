clear;clc;close all;
% load('data\Preprodata_targetPlane_0409.mat');

c = 3e8;
% SNR = 5;
load('data\targetPara_0422.mat');
[f0,lambda,Bw,fs,N,K,df,tb,tc,ts,PRF,Nt,Nr,dt,dr] = MIMORadarPara;
deltaR = c/2/Bw;
L_T = fs*ts;
L_c = fs*tc;
...
for snr_inds = 1:length(SNR_db)
    % SNR_db = -5;
    SNR = 10^(SNR_db(snr_inds)/10);  %10db
...
scatter(xm_est1,ym_est1,S_est1,'*','c')
% 
grid on
xlabel('$x/{\mathrm{m}}$','Interpreter','latex')
ylabel('$y/{\mathrm{m}}$','Interpreter','latex')
...
legend('Original scatterers','With the FFT method', 'With 2D-ESPRIT','With the proposed algorithm')
